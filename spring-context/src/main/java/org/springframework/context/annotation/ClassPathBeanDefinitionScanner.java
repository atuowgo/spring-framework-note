...
public class ClassPathBeanDefinitionScanner extends ClassPathScanningCandidateComponentProvider {
	...
	protected Set<BeanDefinitionHolder> doScan(String... basePackages) {
		...
		for (String basePackage : basePackages) {
			Set<BeanDefinition> candidates = findCandidateComponents(basePackage);//Tip:扫描basePackage路径下所有带有@Component,@Service,@Repository和@Controller的类
			for (BeanDefinition candidate : candidates) {
				...
				if (candidate instanceof AnnotatedBeanDefinition) {
					AnnotationConfigUtils.processCommonDefinitionAnnotations((AnnotatedBeanDefinition) candidate);//Tip:处理常见注解，如@Lazy,@DependencyOn
				}
				if (checkCandidate(beanName, candidate)) {
					...
					registerBeanDefinition(definitionHolder, this.registry);//Tip:注册BeanDefinition到上下文中
				}
			}
		}
	...
	protected boolean checkCandidate(String beanName, BeanDefinition beanDefinition) throws IllegalStateException {
		if (!this.registry.containsBeanDefinition(beanName)) {//Tip:未在上下文中注册过
			return true;
		}
